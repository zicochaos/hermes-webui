...
function setLocale(lang) {
  ...
  if (!LOCALES[resolved] && resolved !== 'en') {
    ...
    // resolveLocale only recognizes already-loaded keys, so a tag whose
    // bundle has not been loaded yet resolves to null here (hence the 'en'
    // fallback above). The language dropdown only ever passes exact LOCALES
    // keys and localStorage stores resolved keys, so the tag's lowercased
    // form is the bundle file name; its onload re-runs setLocale with the
    // real key, and a 404 cleans itself up in onerror leaving English active.
